...
1. **F-RPT-008 样本量规划器**（预估样本量计算：输入效应量/功效/显著性 → 给出 N 建议，与代表性诊断互文）
2. **一句话结论下沉到导出物**（Word/PPT 正文段落引入 one_liner，正文级"说人话"）
3. **报告页 Tabs 化**（样本代表性/体检/诊断分页签，缓解报告页过长）
