...
| 样本代表性诊断（F-RPT-007） | ✅ v1.3 验收 | 免费、纯诊断不卖样本，阈值唯一来源 |
| 样本量规划器（F-RPT-008） | ✅ v1.3.1 验收 | 预演闭环：预演效应量→回收目标，公式引擎无 LLM |
...
| [s-2026-08-10-下阶段验收报告.md](docs/s-2026-08-10-下阶段验收报告.md) | v1.3 | 样本代表性诊断 + 首页叙事 + 一句话结论验收 |
...
1. **一句话结论下沉到导出物**（Word/PPT 正文段落引入 one_liner，正文级"说人话"）
2. **报告页 Tabs 化 + 代表性/规划跨页联动**（报告页分页签，代表性诊断回看规划目标 N 与已收 N 对照）
3. **规划器增强**（ANOVA/配对/分层设计 + 报告页双入口）
